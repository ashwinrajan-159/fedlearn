...
/** INT8 symmetric symmetric quantization. */
...
function compressAndPack(grad) {
    ...
    if (useTopK && useQuant) {
        ...
        let offset = 12;
        for (let i = 0; i < k; i++) { view.setUint32(offset, indices[i], true); offset += 4; }
...
self.onmessage = async (e) => {
    ...
    if (type === "INIT") {
        ...
        if (payload.jobId !== undefined)     jobId       = payload.jobId;
        ...
        try {
            ...
            if (jobId)           features.push(`Job=${jobId}`);
